...
void Frame::draw(Sint16 x, Sint16 y, double zoomx, double zoomy,double angle) const {
  SDL_Surface* tmp = rotozoomSurface(surface, angle, zoomx, zoomy);
  //SDL_Surface* tmp = zoomSurface(surface, zoomx, zoomy, SMOOTHING_ON);
  Sint16 zero = 0;
  Uint16 width = tmp->w;
  Uint16 height = tmp->h;
  SDL_Rect src = { zero, zero, width, height };    
