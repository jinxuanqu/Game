...
void Clock::update() { 
  ...
  unsigned int delay = capFrame();
  ticksSinceLastFrame +=delay;
  sumOfTicks += ticksSinceLastFrame;
  ticksAtLastFrame = clockTicks+delay;
}
...
unsigned int Clock::capFrame() const { 
  if ( capFrameRate ) return 0u;
  unsigned int delay = std::max(0.0,1000.0/frameCap+0.5 - ticksSinceLastFrame);
  SDL_Delay( delay );
  return delay;
}
